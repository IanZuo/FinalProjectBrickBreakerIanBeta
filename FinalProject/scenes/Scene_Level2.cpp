...
void Scene_Level2::SceneStartUp(SDL_Renderer* renderer) {
    Scene::SceneStartUp(renderer);
    mBricks = LevelLoader::LoadLevel("resources/level2.txt");
}

void Scene_Level2::SceneShutDown() {
    mBricks.clear();
}

void Scene_Level2::Input(SDL_Event& e) {
    if (e.type == SDL_KEYDOWN && e.key.keysym.sym == SDLK_n) {
        SetSceneStatus(false);
    }
}

void Scene_Level2::Update(float dt) {
    ...
    if (allDestroyed) {
        std::cout << "[Level2] Complete!\n";
        SetSceneStatus(false);
    }
}

void Scene_Level2::Render() {
    SDL_SetRenderDrawColor(mRenderer, 10, 10, 30, 255);
